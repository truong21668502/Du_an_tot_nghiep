...
<template>
    ...
    <StaffLayout>
        ...
        <div v-if="orders.length === 0"
            class="bg-surface-container-lowest rounded-xl p-10 shadow-soft flex flex-col items-center justify-center border border-outline-variant/30 text-center h-[400px]">
            ...
            <div v-for="order in orders" :key="order.id" @click="openOrderDetails(order)"
                ...
                <div>
                    <div class="flex items-center gap-4 mb-4 border-b border-outline-variant/30 pb-4">
                        ...
                        <div class="flex-1">
                            <h4 class="text-headline-sm text-on-surface truncate">{{ order.table ?
                        ...
                        <span v-if="order.status === 'PENDING'"
                            class="text-label-sm text-error bg-error-container text-on-error-container px-3 py-1.5 rounded-full whitespace-nowrap">Chờ
                            tiếp nhận</span>
                        <span v-else-if="order.status === 'PROCESSING'"
                            class="text-label-sm text-primary bg-primary-container text-on-primary-container px-3 py-1.5 rounded-full whitespace-nowrap">Đang
                            xử lý</span>
                    </div>

                    <ul class="space-y-2 mb-4">
                        <li v-for="detail in order.order_details" :key="detail.id"
                            class="flex justify-between items-start text-body-md">
