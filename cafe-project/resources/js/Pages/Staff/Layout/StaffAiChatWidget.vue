...
const loadConversations = async () => {
    try {
        const response = await axios.get('/nhan-vien/ai-conversations')
...
const loadHistory = async () => {
    ...
    try {
        const response = await axios.get('/nhan-vien/ai-history', {
...
const deleteConversation = async (convId) => {
    ...
    try {
        await axios.delete('/nhan-vien/ai-conversation', {
...
const sendMessage = async () => {
    ...
    try {
        const response = await axios.post('/nhan-vien/ai-chat', { 
            message: message,
...
<template>
    <div class="fixed bottom-6 right-6 z-50">
        ...
        >
            <!-- Header -->
            <div class="flex items-center justify-between p-4 border-b border-outline-variant/20 bg-primary text-white rounded-t-2xl z-20">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="font-sans text-label-md font-semibold uppercase tracking-wider">Trợ lý Nắng Coffee</h3>
                        <p class="text-[11px] opacity-80">Luôn sẵn sàng hỗ trợ</p>
                    </div>
                </div>
            ...
            >
                <div v-if="messages.length === 0 && !isLoading" class="text-center py-12">
                    ...
                    <p class="font-sans text-body-sm text-on-surface-variant">
                        Xin chào! Bạn cần hỗ trợ gì về bàn hay đơn hàng không?
                    </p>
                </div>
            ...
            <div class="p-4 border-t border-outline-variant/20 bg-surface">
                <div class="flex items-end gap-2">
                    <textarea
                        ...
                        placeholder="Hỏi về bàn, đơn hàng..."
                        rows="1"
