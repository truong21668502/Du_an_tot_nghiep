...
const updateReservationStatus = (reservationId, newStatus) => {
    router.patch(route('staff.reservations.update-status', reservationId), { status: newStatus }, {
        preserveScroll: true,
        onSuccess: () => {
            if (selectedTable.value?.reservations) {
                ...
                if (newStatus === 'CANCELLED') {
                    ...
                    selectedTable.value.reservations = selectedTable.value.reservations.filter(r => r.id !== reservationId);
...
const activeReservation = computed(() => {
    ...
    // Ưu tiên PENDING trước — staff cần xử lý cái chưa xác nhận trước
    return selectedTable.value.reservations.find(r => r.status === 'PENDING')
        || selectedTable.value.reservations.find(r => r.status === 'CONFIRMED')
        || null;
});

// Bàn có reservation PENDING — kể cả khi status bàn vẫn EMPTY
const hasPendingReservation = (table) => {
    return table?.reservations?.some(r => r.status === 'PENDING');
};
            ...
            .listen('.TableUpdated', (e) => {
                ...
                if (index !== -1) {
                    tables.value[index] = {
                        ...tables.value[index],
                        status: e.status,
                        reservations: e.reservations ?? tables.value[index].reservations,
                    };
                    if (selectedTable.value?.id === e.id) {
                    ...
                    <div v-for="order in orders" :key="order.id" @click="openOrderDetails(order)"
                        ...
                        <div>
                            <div class="flex items-center gap-4 mb-3 border-b border-outline-variant/30 pb-3">
                                ...
                                <div class="flex-1">
                                    <h4 class="text-label-md text-on-surface truncate">
                                ...
                                <span v-if="order?.status === 'PENDING'"
                                    class="text-label-sm text-error bg-error-container text-on-error-container px-3 py-1 rounded-full whitespace-nowrap">
                                    Chờ xử lý
                                </span>
                                <span v-else-if="order?.status === 'PROCESSING'"
                                    class="text-label-sm text-primary bg-primary-container text-on-primary-container px-3 py-1 rounded-full whitespace-nowrap">
                                    Đang xử lý
                                </span>
                            </div>
            ...
            <section class="xl:col-span-1">
                ...
                <div class="bg-surface-container-lowest rounded-xl p-6 shadow-soft border border-outline-variant/30">
                    <div v-for="(areaTables, areaName) in groupedTables" :key="areaName" class="mb-8 last:mb-0">
                        <h4 class="text-label-md text-outline mb-4 border-b border-outline-variant/30 pb-2">
                            {{ areaName }}
                        </h4>
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                            <div v-for="table in areaTables" :key="table.id" @click="openTableDetails(table)"
                                class="aspect-square rounded-xl flex flex-col items-center justify-center cursor-pointer transition-all border-2 relative"
                                :class="[
                                    table?.status === 'OCCUPIED' ? 'bg-primary-container/20 border-primary text-primary shadow-sm' : '',
                                    table?.status === 'RESERVED' || hasPendingReservation(table) ? 'bg-secondary-container border-transparent text-on-secondary-container shadow-sm font-bold' : '',
                                    table?.status === 'EMPTY' && !hasPendingReservation(table) ? 'bg-surface border-transparent hover:bg-surface-container-high text-on-surface-variant' : '',
                                ]">

                                <!-- Chấm đỏ: có reservation PENDING dù bàn EMPTY hay RESERVED -->
                                <div v-if="hasPendingReservation(table)"
                                    class="absolute top-2 right-2 w-2.5 h-2.5 bg-error rounded-full animate-pulse shadow-[0_0_6px_rgba(186,26,26,0.8)]">
                                </div>
                    ...
                    <div v-if="isOrderModalOpen"
                        ...
                        <div class="px-6 py-5 bg-surface border-t border-outline-variant/30 flex gap-4 justify-end">
                            ...
                            <button v-if="selectedOrder?.status === 'PENDING'" @click="acceptOrder(selectedOrder.id)"
                                class="px-8 py-2.5 rounded-full bg-primary text-on-primary text-label-md shadow-soft hover:bg-primary/90 flex items-center gap-2">
                            ...
                            <button v-else-if="selectedOrder?.status === 'PROCESSING'"
                                ...
                                class="px-8 py-2.5 rounded-full bg-secondary text-on-secondary text-label-md shadow-soft hover:bg-secondary/90 flex items-center gap-2">
            ...
            <div v-if="isTableModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
                ...
                <Transition name="slide-up">
                    <div v-if="isTableModalOpen"
                        class="relative bg-surface-container-lowest rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col">

                        <!-- Header modal — đổi màu theo trạng thái thực tế -->
                        <div class="px-6 py-5 border-b border-outline-variant/30 flex justify-between items-center"
                            :class="[
                                ...
                                selectedTable?.status === 'RESERVED' || hasPendingReservation(selectedTable) ? 'bg-secondary-container text-on-secondary-container' : 'bg-surface text-on-surface',
                            ]">
                            <div>
                        ...
                        <div class="p-6 bg-surface-container-lowest max-h-[75vh] overflow-y-auto hide-scrollbar">
                            ...
                            <!-- Có reservation → hiện thông tin, không cần check status bàn -->
                            <div v-if="activeReservation" class="mb-6 space-y-4">
                                <div
                                    class="bg-secondary-container/10 border-2 border-dashed border-secondary/30 rounded-xl p-4">
                                    <p
                                        class="text-label-sm text-secondary font-bold tracking-wider mb-3 flex items-center gap-1">
                                        <span class="material-symbols-outlined text-[16px]">bookmark_heart</span>
                                        THÔNG TIN ĐẶT TRƯỚC
                                    </p>
                                    <div class="grid grid-cols-1 gap-3 text-body-md">
                                        <div class="flex justify-between border-b border-outline-variant/20 pb-2">
                                            ...
                                            <span class="font-bold text-on-surface">
                                                {{ activeReservation.user?.full_name || '---' }}
                                            </span>
                                        </div>
                                        <div class="flex justify-between border-b border-outline-variant/20 pb-2">
                                            <span class="text-on-surface-variant">Số điện thoại:</span>
                                            <span class="font-bold text-primary">
                                                {{ activeReservation.phone_number || '---' }}
                                            </span>
                                        </div>
                                        <div class="flex justify-between border-b border-outline-variant/20 pb-2">
                                            <span class="text-on-surface-variant">Số khách:</span>
                                            <span class="font-bold text-on-surface">
                                        ...
                                        <div class="flex justify-between border-b border-outline-variant/20 pb-2">
                                            ...
                                            <span
                                                class="text-label-sm font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider"
                                        ...
                                        <div class="pt-1">
                                            <span class="text-on-surface-variant text-label-sm block mb-1">Ghi chú từ
                                                khách:</span>
                                            <p
                                                class="text-body-md italic text-on-surface bg-surface p-2.5 rounded-lg border border-outline-variant/30 min-h-[40px]">
                            ...
                            <div v-else
                                ...
                                <span class="text-label-md font-bold uppercase tracking-wider px-3 py-1 rounded-md"
                                    :class="selectedTable?.status === 'EMPTY'
                                        ? 'bg-surface-dim text-on-surface-variant'
                                        : selectedTable?.status === 'OCCUPIED'
                                            ? 'bg-primary-container text-primary'
                                            : 'bg-secondary-container text-secondary'">
                                    {{ selectedTable?.status === 'EMPTY' ? 'Bàn trống'
                                        : selectedTable?.status === 'OCCUPIED' ? 'Đang có khách'
                                            : 'Đã đặt' }}
                                </span>
                            </div>

                            <div class="space-y-3">
                                <!-- Có PENDING → từ chối / xác nhận (dù bàn EMPTY hay RESERVED) -->
                                <div v-if="activeReservation?.status === 'PENDING'" class="flex gap-3">
                                    <button @click="updateReservationStatus(activeReservation.id, 'CANCELLED')"
                                    ...
                                    <button @click="updateReservationStatus(activeReservation.id, 'CONFIRMED')"
                                        ...
                                        <span class="material-symbols-outlined text-[20px]">check_circle</span> Xác nhận
                                        lịch
                                    </button>
                                </div>

                                <!-- CONFIRMED → cho vào bàn hoặc đánh dấu không đến -->
                                <div v-else-if="activeReservation?.status === 'CONFIRMED'" class="flex gap-3">
                                    <button @click="updateReservationStatus(activeReservation.id, 'CANCELLED')"
                                        class="flex-1 py-3.5 rounded-xl border-2 border-error text-error font-bold hover:bg-error-container transition-colors flex items-center justify-center gap-2">
                                        <span class="material-symbols-outlined text-[20px]">person_off</span> Không đến
                                    </button>
                                    <button @click="updateTableStatus(selectedTable.id, 'OCCUPIED')"
                                        class="flex-1 py-3.5 rounded-xl bg-secondary text-on-secondary text-label-md hover:opacity-90 shadow-sm font-bold flex items-center justify-center gap-2">
                                        <span class="material-symbols-outlined text-[20px]">how_to_reg</span> Đã đến
                                    </button>
                                </div>

                                <!-- Không có reservation + bàn trống → khách vãng lai -->
                                <button v-else-if="selectedTable?.status === 'EMPTY' && !activeReservation"
                                    ...
                                    <span class="material-symbols-outlined text-[20px]">login</span> Khách vãng lai vào
                                    bàn
                                </button>
