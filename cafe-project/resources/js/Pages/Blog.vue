    ...
    <div class="max-w-[1280px] mx-auto px-margin-mobile md:px-gutter py-12 md:py-24">
      <AnimateOnScroll animation="fade-up" :duration="700">
        <div class="mb-12 text-center">
          <span
            class="inline-block px-4 py-2 bg-primary-container/30 text-on-primary-container rounded-full font-sans text-label-sm mb-4">Blog</span>
          <h1 class="text-display-lg-mobile md:text-display-lg text-primary mb-3">Tin tức & Câu chuyện</h1>
          <p class="text-body-lg text-on-surface-variant max-w-2xl mx-auto">Khám phá những câu chuyện thú vị về cà phê,
            văn hóa và con người tại Nắng Coffee.</p>
        </div>
      </AnimateOnScroll>
      <BlogHeader v-model:search="filters.search" :categories="categories" :active-category="filters.category"
        @update:search="setSearch" @update:category="setCategory" />
      ...
      <BlogPagination v-if="totalPages > 1" :current-page="filters.page" :total-pages="totalPages"
        @page-change="setPage" />
    </div>
  </div>
</template>
