...
const props = defineProps({
    form:   { type: Object, required: true }, // instance useForm() từ trang cha
    isEdit: { type: Boolean, default: false },
})
const emit = defineEmits(['submit'])

// ── Ảnh: preview tức thời từ File mới chọn, hoặc image_url có sẵn khi edit ──────
...
// ── Banner "sống" dùng cho preview — đổi field nào, preview đổi ngay field đó ──
...
<template>
    <form @submit.prevent="emit('submit')" class="grid lg:grid-cols-2 gap-8 items-start">

        <!-- ── Cột trái: Live Preview ── -->
        <div class="lg:sticky lg:top-6 space-y-3">
            <p class="font-sans text-label-sm text-on-surface-variant">Xem trước</p>
            <Banner :banner="previewBanner" preview />
        </div>
