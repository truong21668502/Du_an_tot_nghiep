...
const { loading, errors, login, loginWithGoogle } = useAuth()
const form = ref({
  email: '',
  password: '',
  remember: false
  ...
  <div class="min-h-screen flex items-center justify-center px-margin-mobile py-12 bg-surface-container-low">
    <div class="w-full max-w-md">
      <div class="text-center mb-10">
        ...
        <h1 class="font-serif text-headline-sm text-on-surface">Dang Nhap</h1>
        <p class="font-sans text-body-md text-on-surface-variant mt-2">
          Chao mung ban tro lai! Vui long dang nhap de tiep tuc.
        </p>
      </div>
      <div class="bg-surface rounded-2xl shadow-soft border border-outline-variant/20 p-8">
          ...
          <div class="flex justify-between items-center">
            <label class="flex items-center gap-2 cursor-pointer">
              <input
                ...
                type="checkbox"
                class="w-4 h-4 rounded border-outline-variant/50 text-primary focus:ring-secondary/30 cursor-pointer"
              />
              <span class="font-sans text-label-sm text-on-surface-variant">Ghi nho dang nhap</span>
            </label>
            <Link href="/quen-mat-khau" class="font-sans text-label-sm text-secondary hover:text-secondary/80 transition-colors">
              Quen mat khau?
            </Link>
          </div>
        ...
        <p class="text-center mt-6 font-sans text-body-md text-on-surface-variant">
          Chua co tai khoan?
          <Link href="/dang-ky" class="text-secondary font-semibold hover:text-secondary/80 transition-colors ml-1">
            Dang ky ngay
          </Link>
        </p>
      </div>
