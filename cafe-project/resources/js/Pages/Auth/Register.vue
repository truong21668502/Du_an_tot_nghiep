...
const errors = ref({})
const form = ref({ full_name: '', email: '', password: '', password_confirmation: '', agree_terms: false })
const showPassword = ref(false)
const showConfirmPassword = ref(false)
const handleRegister = () => {
  if (!form.value.agree_terms) {
    errors.value = { agree_terms: 'Bạn phải đồng ý với điều khoản sử dụng' }
    return
  }
  loading.value = true
...
const handleCustomGoogleClick = () => {
    const realGoogleBtn = document.querySelector('#googleButtonBtn div[role="button"]')
...
<template>
  <div class="min-h-screen flex items-center justify-center px-margin-mobile py-12 bg-surface-container-low">
    <div class="w-full max-w-md">
      ...
      <div class="bg-surface rounded-2xl shadow-soft border border-outline-variant/20 p-8">
        <form @submit.prevent="handleRegister" class="space-y-5">
          <div class="space-y-2">
            <label for="full_name" class="block font-sans text-label-sm text-on-surface">Họ và tên <span class="text-error">*</span></label>
            <div class="relative">
              <span class="absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant/60"><span class="material-symbols-outlined text-lg">person</span></span>
              <input id="full_name" v-model="form.full_name" type="text" required placeholder="Nguyễn Văn A" class="w-full pl-12 pr-4 py-3 bg-surface-container-low border border-outline-variant/30 rounded-xl font-sans text-body-md text-on-surface placeholder:text-outline focus:outline-none focus:border-secondary focus:ring-2 focus:ring-secondary/20 transition-all duration-300" />
            </div>
          ...
          <div class="space-y-2">
            <label for="password_confirmation" class="block font-sans text-label-sm text-on-surface">Xác nhận mật khẩu <span class="text-error">*</span></label>
            <div class="relative">
              ...
              <input id="password_confirmation" v-model="form.password_confirmation" :type="showConfirmPassword ? 'text' : 'password'" required placeholder="Nhập lại mật khẩu" class="w-full pl-12 pr-12 py-3 bg-surface-container-low border border-outline-variant/30 rounded-xl font-sans text-body-md text-on-surface placeholder:text-outline focus:outline-none focus:border-secondary focus:ring-2 focus:ring-secondary/20 transition-all duration-300" />
            </div>
          </div>
          <label class="flex items-start gap-2 cursor-pointer">
            <input v-model="form.agree_terms" type="checkbox" class="mt-0.5 w-4 h-4 rounded border-outline-variant/50 text-primary focus:ring-secondary/30 cursor-pointer" />
            <span class="font-sans text-label-sm text-on-surface-variant">Tôi đồng ý với <Link href="#" class="text-secondary hover:underline">Điều khoản sử dụng</Link> và <Link href="#" class="text-secondary hover:underline">Chính sách bảo mật</Link></span>
          </label>
      ...
      <a @click="handleCustomGoogleClick" class="cursor-pointer w-full flex items-center justify-center gap-3 py-3 px-4 bg-surface-container-low hover:bg-surface-container border border-outline-variant/30 rounded-xl font-sans text-label-md text-on-surface transition-all duration-300 hover:shadow-sm">
    ...
    <div class="w-full flex justify-center min-h-0 ">
      <div id="googleButtonBtn" class="absolute invisible pointer-events-none"></div>
    </div>

        <p class="text-center mt-6 font-sans text-body-md text-on-surface-variant">
          Đã có tài khoản? <Link href="/dang-nhap" class="text-secondary font-semibold hover:text-secondary/80 transition-colors ml-1">Đăng nhập ngay</Link>
        </p>
      </div>
    </div>
  </div>
