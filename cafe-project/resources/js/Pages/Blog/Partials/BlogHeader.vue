...
<template>
  <div class="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between mb-12">
    <div class="w-full md:w-80">
      <SearchBar :model-value="search" placeholder="Tìm kiếm bài viết..."
        @update:model-value="$emit('update:search', $event)" />
    </div>
    <div class="flex flex-wrap gap-2">

      <button @click="$emit('update:category', 'all')" :class="[
      ...
      <button v-for="cat in categories" :key="cat.id" @click="$emit('update:category', cat.slug || cat.id)" :class="[
        'px-4 py-2 rounded-full font-sans text-label-sm border transition-all duration-300',
