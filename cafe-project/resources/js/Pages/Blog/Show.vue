    ...
    <article class="max-w-[900px] mx-auto px-margin-mobile md:px-gutter py-12 md:py-24">
      <AnimateOnScroll animation="fade-up" :duration="700">
        <button @click="goBack" type="button"
          class="inline-flex items-center gap-1 mb-6 font-sans text-label-md text-on-surface-variant hover:text-primary transition-colors">
          <span class="material-symbols-outlined text-base">arrow_back</span>
          Quay lại
        </button>
        <div class="mb-8">
          <span
            class="inline-block px-3 py-1 bg-primary-container/30 text-on-primary-container rounded-full font-sans text-label-sm mb-4">
          ...
          <div class="flex flex-wrap items-center gap-4 text-on-surface-variant font-sans text-label-sm">
            <span class="flex items-center gap-1"><span class="material-symbols-outlined text-sm">person</span> {{
              post.author?.name || 'Nắng Coffee' }}</span>
            <span class="flex items-center gap-1"><span class="material-symbols-outlined text-sm">calendar_today</span>
              {{ formatDate(post.published_at || post.created_at) }}</span>
            <span class="flex items-center gap-1"><span class="material-symbols-outlined text-sm">schedule</span> {{
      ...
      <AnimateOnScroll animation="fade-up" :duration="700" :delay="300">
        <div class="prose prose-lg max-w-none font-sans text-body-lg text-on-surface leading-relaxed space-y-6"
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 mt-10 pt-8 border-t border-outline-variant/20">
          <div class="flex flex-wrap gap-2">
            <span v-for="tag in (post.tags || [])" :key="tag"
              class="px-3 py-1 bg-surface-container-low border border-outline-variant/20 rounded-full font-sans text-label-sm text-on-surface-variant">{{
                tag }}</span>
          </div>
          <div class="flex items-center gap-2">
            <span class="font-sans text-label-sm text-on-surface-variant">Chia sẻ:</span>
            <a v-for="link in shareLinks" :key="link.label" :href="link.url" target="_blank" rel="noopener noreferrer"
              class="w-9 h-9 rounded-full bg-surface-container-low hover:bg-primary-container/30 text-on-surface-variant hover:text-primary flex items-center justify-center transition-all">
