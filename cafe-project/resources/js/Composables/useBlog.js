...
export function useBlog(initialPosts = [], initialCategories = [], initialFilters = {}) {
  ...
  const filteredPosts = computed(() => {
    ...
    if (filters.value.search.trim()) {
      ...
      result = result.filter(p => p.title.toLowerCase().includes(term) || p.excerpt?.toLowerCase().includes(term))
    }
    if (filters.value.category !== 'all') {
      result = result.filter(p => p.category_id == filters.value.category || p.category?.slug === filters.value.category)
    }
    return result
  })
  ...
  const fetchPosts = () => {
    loading.value = true
    router.get('/bai-viet', filters.value, {
      preserveScroll: true,
      preserveState: true,
  ...
  return {
    posts, categories, loading, errors, filters, totalPosts, totalPages, filteredPosts,
    setSearch, setCategory, setPage, fetchPosts, formatDate
  }
}
